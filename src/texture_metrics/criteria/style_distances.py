...
def gramm(
    tnsr: torch.Tensor | Iterable[torch.Tensor], center_gram=True
) -> torch.Tensor:
    ...
    for i, (input, target) in enumerate(zip(inputs, targets)):
        if func == "mean":
            ...
            distances[:, i] = mse_loss(
                gramm(input, center_gram=False), gramm(target, center_gram=False)
            )
            ...
            distances[:, i] = sliced_wasserstein_distance(
                input, target, nslice=kwargs.get("sstyle") * c, batch_size=c
            )
            print(distances.untyped_storage().nbytes() / (1024**3))
            print("swd", c, torch.cuda.device_memory_used() / (1024**3))
        elif callable(func):
            distances[:, i] = mse_loss(func(input, **kwargs), func(target, **kwargs))
